...
class EquiformerV2_OC20_2output_Backbone(BaseModel):
    """
    Equiformer with graph attention built upon SO(2) convolution and feedforward network built upon S2 activation
    ...
    def __init__(
        ...
        # Output blocks for energy and forces
        self.norm1 = get_normalization_layer(self.norm_type, lmax=max(self.lmax_list), num_channels=self.sphere_channels)
        self.norm = get_normalization_layer(self.norm_type, lmax=max(self.lmax_list), num_channels=self.sphere_channels)
        self.apply(self._init_weights)
        self.apply(self._uniform_init_rad_func_linear_weights)
    ...
    def forward(self, data):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers-2:
                old_x = self.norm1(x.embedding).reshape(num_atoms,-1)

        # Final layer norm
        x.embedding = self.norm(x.embedding)
        # print(old_x.shape,x.to_e3nn_embeddings()[1].shape)
        return torch.cat([old_x,x.to_e3nn_embeddings()[1]],dim = 1)
